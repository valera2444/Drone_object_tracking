...
class Engine(object):
    ...
    def train(self, print_freq=10, fixbase_epoch=0, open_layers=None):
        if self.model.loss == 'triplet':
            if self.datamanager.batch_size_train < 3*self.datamanager.num_instances:            
                warnings.warn(f"If you are using {self.model.loss} loss, your batch_size must be more than 3*self.datamanager.num_instances. Otherwise it's not guaranteed that triplets will be formed correctly")
